# Generated by Django 5.1.6 on 2025-04-24 10:04

import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('hesaplama', '0018_kargoucretgecmis'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='KomisyonOraniGecmisi',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('email', models.EmailField(max_length=254)),
                ('komisyon_orani', models.DecimalField(decimal_places=2, max_digits=5)),
                ('hesaplama_tarihi', models.DateTimeField(auto_now_add=True)),
                ('kategori_yolu', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='hesaplama.hesaplamakategoriler')),
                ('kullanici', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
                ('pazar_yeri', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='hesaplama.pazaryeri')),
            ],
            options={
                'verbose_name': 'Komisyon Oranı Geçmişi',
                'verbose_name_plural': 'Komisyon Oranı Geçmişleri',
                'ordering': ['-hesaplama_tarihi'],
            },
        ),
    ]
